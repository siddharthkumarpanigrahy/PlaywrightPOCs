...
SPECIAL_CHAR_DIR = (
    "./test_data/mc_gui/security/"
    "margin calculator/special_characters"
)

SPECIAL_CHAR_FILE = (
    SPECIAL_CHAR_DIR +
    "/TC-012_SpecialCharacters.csv"
)
...
def prepare_special_character_file():
    ...
    log(
        "Creating special-character CSV file from base file: "
        f"[{BASE_CSV_FILE}]"
    )
    ...
    # Inject special characters into free-text style field.
    # This is intentionally security-focused input.
    data_rows[0][internal_trade_id_index] = SPECIAL_CHAR_VALUE
    ...
    if not os.path.exists(SPECIAL_CHAR_FILE):
        raise Exception(
            f"Special-character file was not created: {SPECIAL_CHAR_FILE}"
        )

    file_size = os.path.getsize(SPECIAL_CHAR_FILE)

    log(
        "Created special-character file successfully. "
        f"path=[{SPECIAL_CHAR_FILE}], "
        f"size_bytes=[{file_size}], "
        f"injected_value=[{SPECIAL_CHAR_VALUE}]"
    )

    return SPECIAL_CHAR_FILE
    ...
    for message in popup_messages:
        try:
            if page.get_by_text(
                ...
                page.screenshot(
                    path="runtime/screenshots/mc_tc006_initial_popup.png"
                )
...
def login_to_mc_gui(page):
    ...
    page.screenshot(
        path="runtime/screenshots/mc_tc006_before_login.png"
    )
    ...
    if username_value != MC_USERNAME:
        page.screenshot(
            path="runtime/screenshots/mc_tc006_username_not_entered.png"
        )
    ...
    if not password_value:
        page.screenshot(
            path="runtime/screenshots/mc_tc006_password_not_entered.png"
        )

        raise Exception("Password was not entered")

    page.screenshot(
        path="runtime/screenshots/mc_tc006_credentials_entered.png"
    )
    ...
    try:
        ...
        page.screenshot(
            path="runtime/screenshots/mc_tc006_login_button_still_disabled.png"
        )
    ...
    page.screenshot(
        path="runtime/screenshots/mc_tc006_after_login.png"
    )

    handle_initial_popup_if_present(page)


def upload_special_character_file(page, file_path):
    log(f"Uploading special-character file [{file_path}]")
    ...
    page.screenshot(
        path="runtime/screenshots/mc_tc006_after_special_character_upload.png"
    )
...
def click_validate_button(page):
    ...
    page.screenshot(
        path="runtime/screenshots/mc_tc006_after_validate_click.png"
    )


def validate_special_character_result(page):
    log("Validating MC-GUI special-character handling result")
    ...
    page.screenshot(
        path="runtime/screenshots/mc_tc006_special_character_validation.png"
    )
    ...
    if not matched_messages:
        raise Exception(
            "Expected special-character validation/result message "
            "was not found after clicking Validate"
        )

    log(
        "Special-character upload produced controlled result. "
        f"Matched messages={matched_messages}"
    )

    # If the application shows an error grid, validate the grid is controlled.
    ...
    page.screenshot(
        path="runtime/screenshots/mc_tc006_expected_result_verified.png"
    )

    log(
        "OTCT-7968_TC_006 PASSED - MC-GUI handled special-character "
...
def logout_mc_gui(page):
    ...
    try:
        ...
        page.screenshot(
            path="runtime/screenshots/mc_tc006_before_logout.png"
        )
        ...
        page.screenshot(
            path="runtime/screenshots/mc_tc006_after_logout.png"
        )
...
try:
    log("Starting MC-GUI OTCT-7968_TC_006")

    special_character_file = prepare_special_character_file()

    login_to_mc_gui(page)

    upload_special_character_file(
        page,
        special_character_file
    )

    click_validate_button(page)

    validate_special_character_result(page)
