...
LARGE_FILE = (
    ...
    "portfolio transfer/Portfolio_50000_TradeIds.csv"
)
...
def generate_50000_trade_id_file(file_path):
    """
    Generates exactly 50,000 trade rows plus one header row.
    File format:
    ...
    log(f"Generating 50,000 Trade ID CSV file at [{file_path}]")

    with open(file_path, "w", encoding="utf-8", newline="") as file:
        file.write("CCPTradeId,Fee,PartialTransferAmount\n")

        for index in range(1, RECORD_COUNT + 1):
            trade_id = 700000000 + index
    ...
    log(
        f"Generated file successfully. "
        f"Data rows=[{actual_rows}], size_bytes=[{file_size}]"
    )
...
def wait_for_progress_bar_close(page, timeout=180000):
...
def confirm_mtm_if_present(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc032_mtm_confirmation_popup.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc032_mtm_popup_no_button_found.png"
    )
...
def check_storage_error_after_upload(page):
    if page.get_by_text(
        ...
        page.screenshot(
            path="runtime/screenshots/tc032_file_not_found_in_storage.png"
        )

        raise Exception(
            "File upload failed before 50,000-record validation: "
            "File not found in storage"
        )


def validate_large_file_result(page):
    ...
    # Case 1: explicit validation popup/message.
    if any(keyword in body_text_lower for keyword in VALIDATION_KEYWORDS):
        if (
            ...
            page.screenshot(
                path="runtime/screenshots/tc032_validation_popup_displayed.png"
            )

            log(
                "50,000-record file triggered validation popup/message"
            )
    ...
    if result_row_count > 0:
        ...
        if upload_status in ("SUCCESS", "FAILURE"):
            page.screenshot(
                path="runtime/screenshots/tc032_grid_result_row.png"
            )

            log(
                "50,000-record file was handled by application "
        ...
        raise Exception(
            "50,000-record file produced grid row, "
            f"but upload status was unexpected: [{upload_status}]"
        )

    page.screenshot(
        path="runtime/screenshots/tc032_no_validation_or_grid_result.png"
    )

    raise Exception(
        "No validation popup and no result grid row found "
        "for 50,000-record upload"
    )
...
try:
    log("Starting OTCT-7968_TC_032")

    # --------------------------------------------------
    # Generate physical 50,000 Trade ID file
    # --------------------------------------------------

    generate_50000_trade_id_file(LARGE_FILE)

    if not os.path.exists(LARGE_FILE):
        raise Exception(
            f"Missing generated file: {LARGE_FILE}"
        )
    ...
    page.screenshot(
        path="runtime/screenshots/tc032_after_login.png"
    )
    ...
    # --------------------------------------------------
    # Upload 50,000 Trade ID CSV File
    # --------------------------------------------------

    log("Preparing to upload 50,000 Trade ID file")

    upload_start = time.time()

    page.locator(
        PortfolioTransferLocators.FILE_UPLOAD
    ).set_input_files(LARGE_FILE)
    ...
    page.screenshot(
        path="runtime/screenshots/tc032_after_50000_file_upload.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc032_after_book_auto_population.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc032_after_mtm_yes_selection.png"
    )
    ...
    log("Creating Portfolio Transfer for 50,000 records")
    ...
    process_elapsed = round(time.time() - process_start, 2)
    ...
    page.screenshot(
        path="runtime/screenshots/tc032_after_create_click.png"
    )

    # --------------------------------------------------
    # Validate 50,000 Record Handling
    # --------------------------------------------------

    validation_result = validate_large_file_result(page)

    log(
        "OTCT-7968_TC_032 PASSED - Exactly 50,000 Trade IDs "
        "were handled without application crash/hang. "
