...
LARGE_FILE = (
    ...
    "portfolio transfer/Portfolio_49999_TradeIds.csv"
)
...
def generate_49999_trade_id_file(file_path):
    """
    Generates exactly 49,999 trade rows plus one header row.
    File format:
    ...
    log(f"Generating 49,999 Trade ID CSV file at [{file_path}]")

    with open(file_path, "w", encoding="utf-8", newline="") as file:
        file.write("CCPTradeId,Fee,PartialTransferAmount\n")

        for index in range(1, RECORD_COUNT + 1):
            trade_id = 710000000 + index
...
def confirm_mtm_if_present(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc033_mtm_confirmation_popup.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc033_mtm_popup_no_button_found.png"
    )
...
def check_storage_error_after_upload(page):
    if page.get_by_text(
        ...
        page.screenshot(
            path="runtime/screenshots/tc033_file_not_found_in_storage.png"
        )

        raise Exception(
            "File upload failed before 49,999-record validation: "
            "File not found in storage"
        )
...
def validate_large_file_result(page):
    ...
    if any(keyword in body_text_lower for keyword in VALIDATION_KEYWORDS):
        if (
            ...
            page.screenshot(
                path="runtime/screenshots/tc033_validation_popup_displayed.png"
            )

            log(
                "49,999-record file triggered validation popup/message"
            )
    ...
    if result_row_count > 0:
        ...
        if upload_status in ("SUCCESS", "FAILURE"):
            page.screenshot(
                path="runtime/screenshots/tc033_grid_result_row.png"
            )

            log(
                "49,999-record file was handled by application "
        ...
        raise Exception(
            "49,999-record file produced grid row, "
            f"but upload status was unexpected: [{upload_status}]"
        )

    page.screenshot(
        path="runtime/screenshots/tc033_no_validation_or_grid_result.png"
    )

    raise Exception(
        "No validation popup and no result grid row found "
        "for 49,999-record upload"
    )
...
try:
    log("Starting OTCT-7968_TC_033")

    # --------------------------------------------------
    # Generate physical 49,999 Trade ID file
    # --------------------------------------------------

    generate_49999_trade_id_file(LARGE_FILE)
    ...
    page.screenshot(
        path="runtime/screenshots/tc033_after_login.png"
    )
    ...
    # --------------------------------------------------
    # Upload 49,999 Trade ID CSV File
    # --------------------------------------------------

    log("Preparing to upload 49,999 Trade ID file")
    ...
    page.screenshot(
        path="runtime/screenshots/tc033_after_49999_file_upload.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc033_after_book_auto_population.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc033_after_mtm_yes_selection.png"
    )
    ...
    log("Creating Portfolio Transfer for 49,999 records")
    ...
    page.screenshot(
        path="runtime/screenshots/tc033_after_create_click.png"
    )

    # --------------------------------------------------
    # Validate 49,999 Record Handling
    # --------------------------------------------------

    validation_result = validate_large_file_result(page)

    log(
        "OTCT-7968_TC_033 PASSED - 49,999 Trade IDs "
