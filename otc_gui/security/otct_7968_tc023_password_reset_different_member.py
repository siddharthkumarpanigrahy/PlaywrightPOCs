...
def click_user_management_if_available(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc026_after_user_management_navigation.png"
    )
...
def open_password_reset_screen(page):
    ...
    if reset_button is None:
        ...
        page.screenshot(
            path="runtime/screenshots/tc026_password_reset_button_not_found.png"
        )
...
def enter_target_user(page, target_user):
    if not target_user:
        raise Exception(
            "RESET_TARGET_USER_DIFFERENT_MEMBER is missing. "
            "Set RESET_TARGET_USER_DIFFERENT_MEMBER to a different-Member-ID test user."
        )

    log(f"Entering different-member reset target user [{target_user}]")
    ...
    if target_field is None:
        ...
        page.screenshot(
            path="runtime/screenshots/tc026_target_user_field_not_found.png"
        )
...
def fill_manual_password_if_fields_exist(page, new_password):
    ...
    page.screenshot(
        path="runtime/screenshots/tc026_password_fields_entered.png"
    )
...
def click_generate_password_if_available(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc026_after_generate_password_click.png"
    )
...
def submit_password_reset(page):
    ...
    if submit_button is None:
        ...
        page.screenshot(
            path="runtime/screenshots/tc026_submit_button_not_found.png"
        )
    ...
    page.screenshot(
        path="runtime/screenshots/tc026_after_password_reset_submit.png"
    )


def assert_password_reset_blocked(page):
    log("Validating different-member password reset restriction")
    ...
    if body_contains_any(
        ...
        page.screenshot(
            path="runtime/screenshots/tc026_unexpected_success_detected.png"
        )

        raise Exception(
            "Password reset appears successful, but TC026 expects block "
            "for different Member ID target user"
        )
    ...
    if not body_contains_any(
        ...
        log_body_text(
            page,
            "EXPECTED BLOCK MESSAGE NOT FOUND"
        )

        page.screenshot(
            path="runtime/screenshots/tc026_block_message_not_found.png"
        )

        raise Exception(
            "Expected authorization or different-Member-ID block message was not found"
        )

    page.screenshot(
        path="runtime/screenshots/tc026_password_reset_blocked.png"
    )

    log(
        "OTCT-7968_TC_026 PASSED - Password reset for different "
        "Member ID user was blocked with authorization validation"
    )
...
try:
    ...
    # --------------------------------------------------
    # Enter Different-Member Target User
    # --------------------------------------------------
    ...
    assert_password_reset_blocked(page)
