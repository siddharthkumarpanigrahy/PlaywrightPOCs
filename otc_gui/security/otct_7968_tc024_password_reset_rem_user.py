...
RESET_TARGET_REM_USER = os.getenv(
    "RESET_TARGET_REM_USER",
    ""
)
...
def login_as_user(page, username, password):
    ...
    page.screenshot(
        path="runtime/screenshots/tc027_before_login.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc027_after_login.png"
    )
...
def confirm_if_present(page, screenshot_prefix):
    ...
    if (
        "confirm" not in body_text_lower
        and "are you sure" not in body_text_lower
        and "reset" not in body_text_lower
    ):
...
def click_user_management_if_available(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc027_after_user_management_navigation.png"
    )
...
def open_password_reset_screen(page):
    ...
    if reset_button is None:
        ...
        page.screenshot(
            path="runtime/screenshots/tc027_password_reset_button_not_found.png"
        )
    ...
    page.screenshot(
        path="runtime/screenshots/tc027_password_reset_screen_opened.png"
    )
...
def enter_target_user(page, target_user):
    if not target_user:
        raise Exception(
            "RESET_TARGET_REM_USER is missing. "
            "Set RESET_TARGET_REM_USER to a REM test user."
        )

    log(f"Entering REM reset target user [{target_user}]")
    ...
    if target_field is None:
        ...
        page.screenshot(
            path="runtime/screenshots/tc027_target_user_field_not_found.png"
        )
    ...
    page.screenshot(
        path="runtime/screenshots/tc027_target_user_entered.png"
    )
...
def fill_manual_password_if_fields_exist(page, new_password):
    ...
    page.screenshot(
        path="runtime/screenshots/tc027_password_fields_entered.png"
    )
...
def click_generate_password_if_available(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc027_after_generate_password_click.png"
    )
...
def submit_password_reset(page):
    ...
    if submit_button is None:
        ...
        page.screenshot(
            path="runtime/screenshots/tc027_submit_button_not_found.png"
        )
    ...
    page.screenshot(
        path="runtime/screenshots/tc027_after_password_reset_submit.png"
    )


def assert_rem_password_reset_blocked(page):
    log("Validating REM user password reset restriction")
    ...
    if body_contains_any(
        ...
        page.screenshot(
            path="runtime/screenshots/tc027_unexpected_success_detected.png"
        )

        raise Exception(
            "Password reset appears successful, but TC027 expects "
            "block for REM target user"
        )
    ...
    if not body_contains_any(
        ...
        log_body_text(
            page,
            "EXPECTED REM BLOCK MESSAGE NOT FOUND"
        )

        page.screenshot(
            path="runtime/screenshots/tc027_rem_block_message_not_found.png"
        )

        raise Exception(
            "Expected REM user password reset block message was not found"
        )

    page.screenshot(
        path="runtime/screenshots/tc027_rem_password_reset_blocked.png"
    )

    log(
        "OTCT-7968_TC_027 PASSED - Password reset for REM user "
        "was rejected with validation or authorization message"
    )
...
try:
    ...
    assert_rem_password_reset_blocked(page)
