...
UNICODE_EMOJI_FILE = (
    ...
    "portfolio transfer/Portfolio_UnicodeEmoji.csv"
)
...
def confirm_mtm_if_present(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc007_mtm_confirmation_popup.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc007_mtm_popup_no_button_found.png"
    )
...
def check_storage_error_after_upload(page):
    if page.get_by_text(
        ...
        page.screenshot(
            path="runtime/screenshots/tc007_file_not_found_in_storage.png"
        )

        raise Exception(
            "File upload failed before Unicode/emoji validation: "
            "File not found in storage"
        )


def validate_unicode_emoji_handling(page):
    ...
    if any(keyword in body_text_lower for keyword in VALIDATION_KEYWORDS):
        if (
            ...
            page.screenshot(
                path="runtime/screenshots/tc007_validation_popup_displayed.png"
            )

            log(
                "Unicode/emoji file triggered validation popup/message"
            )
    ...
    if result_row_count > 0:
        ...
        if upload_status in ("SUCCESS", "FAILURE"):
            page.screenshot(
                path="runtime/screenshots/tc007_grid_result_row.png"
            )

            log(
                "Unicode/emoji file was handled safely "
        ...
        raise Exception(
            "Unicode/emoji file produced grid row, "
            f"but upload status was unexpected: [{upload_status}]"
        )

    page.screenshot(
        path="runtime/screenshots/tc007_no_validation_or_grid_result.png"
    )

    raise Exception(
        "No validation popup and no result grid row found "
        "for Unicode/emoji input file"
    )
...
try:
    log("Starting OTCT-7968_TC_007")

    if not os.path.exists(UNICODE_EMOJI_FILE):
        raise Exception(
            f"Missing test data file: {UNICODE_EMOJI_FILE}"
        )
    ...
    page.screenshot(
        path="runtime/screenshots/tc007_after_login.png"
    )
    ...
    # --------------------------------------------------
    # Upload Unicode/Emoji CSV File
    # --------------------------------------------------

    log("Preparing to upload Unicode/emoji file")

    log("Uploading Unicode/emoji file")

    page.locator(
        PortfolioTransferLocators.FILE_UPLOAD
    ).set_input_files(UNICODE_EMOJI_FILE)
    ...
    page.screenshot(
        path="runtime/screenshots/tc007_after_unicode_emoji_file_upload.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc007_after_book_auto_population.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc007_after_mtm_yes_selection.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc007_after_create_click.png"
    )
    ...
    validation_result = validate_unicode_emoji_handling(page)

    log(
        "TC007 PASSED - Unicode/emoji input was handled safely. "
