...
HYPERLINK_OBJECT_FILE = (
    ...
    "portfolio transfer/Portfolio_HyperlinksObjects.csv"
)
...
def confirm_mtm_if_present(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc009_mtm_confirmation_popup.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc009_mtm_popup_no_button_found.png"
    )
...
def check_storage_error_after_upload(page):
    if page.get_by_text(
        ...
        page.screenshot(
            path="runtime/screenshots/tc009_file_not_found_in_storage.png"
        )

        raise Exception(
            "File upload failed before hyperlink/object validation: "
            "File not found in storage"
        )


def validate_hyperlink_object_handling(page):
    ...
    if any(keyword in body_text_lower for keyword in VALIDATION_KEYWORDS):
        if (
            ...
            page.screenshot(
                path="runtime/screenshots/tc009_validation_popup_displayed.png"
            )

            log(
                "Hyperlink/object file triggered validation popup/message"
            )
    ...
    if result_row_count > 0:
        ...
        if upload_status in ("SUCCESS", "FAILURE"):
            page.screenshot(
                path="runtime/screenshots/tc009_grid_result_row.png"
            )

            log(
                "Hyperlink/object file was handled safely "
        ...
        raise Exception(
            "Hyperlink/object file produced grid row, "
            f"but upload status was unexpected: [{upload_status}]"
        )

    page.screenshot(
        path="runtime/screenshots/tc009_no_validation_or_grid_result.png"
    )

    raise Exception(
        "No validation popup and no result grid row found "
        "for hyperlink/object input file"
    )
...
try:
    log("Starting OTCT-7968_TC_009")

    if not os.path.exists(HYPERLINK_OBJECT_FILE):
        raise Exception(
            f"Missing test data file: {HYPERLINK_OBJECT_FILE}"
        )
    ...
    page.screenshot(
        path="runtime/screenshots/tc009_after_login.png"
    )
    ...
    # --------------------------------------------------
    # Upload Hyperlink/Object CSV File
    # --------------------------------------------------

    log("Preparing to upload hyperlink/object file")

    log("Uploading hyperlink/object file")

    page.locator(
        PortfolioTransferLocators.FILE_UPLOAD
    ).set_input_files(HYPERLINK_OBJECT_FILE)
    ...
    page.screenshot(
        path="runtime/screenshots/tc009_after_hyperlink_object_file_upload.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc009_after_book_auto_population.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc009_after_mtm_yes_selection.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc009_after_create_click.png"
    )

    # --------------------------------------------------
    # Validate Hyperlink/Object Handling
    # --------------------------------------------------

    validation_result = validate_hyperlink_object_handling(page)

    log(
        "TC009 PASSED - Hyperlink/object input was handled safely. "
