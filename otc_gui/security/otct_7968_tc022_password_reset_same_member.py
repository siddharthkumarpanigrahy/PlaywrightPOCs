...
def login_as_password_reset_user(page, username, password):
    ...
    page.screenshot(
        path="runtime/screenshots/tc022_before_login.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc022_after_login.png"
    )
...
def open_password_reset_screen(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc022_password_reset_screen_opened.png"
    )


def enter_same_member_target_user(page, target_user):
    if not target_user:
        raise Exception(
    ...
    log(f"Entering same-Member-ID target user [{target_user}]")
    ...
    page.screenshot(
        path="runtime/screenshots/tc022_target_user_entered.png"
    )
...
def assert_no_member_authorization_block(page):
    log("Checking that same-Member-ID target user is not blocked")
    ...
    if body_contains_any(
        ...
        log_body_text(
            page,
            "BLOCK MESSAGE DETECTED"
        )

        page.screenshot(
            path="runtime/screenshots/tc022_same_member_block_detected.png"
        )

        raise Exception(
            "Same-Member-ID target user appears blocked. "
            "TC022 expects the user to be eligible for password reset."
        )
...
def generate_password(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc022_after_generate_password_click.png"
    )


def read_generated_password(page):
    log("Reading generated password")
    ...
    if not generated_password:
        page.screenshot(
            path="runtime/screenshots/tc022_generated_password_empty.png"
        )
...
def populate_confirm_password(page, generated_password):
    ...
    page.screenshot(
        path="runtime/screenshots/tc022_generated_password_confirmed.png"
    )
...
def logout_user(page):
    ...
    try:
        ...
        page.screenshot(
            path="runtime/screenshots/tc022_after_logout.png"
        )
...
try:
    ...
    # --------------------------------------------------
    # Enter Same-Member-ID Target User
    # --------------------------------------------------

    enter_same_member_target_user(
        page,
        RESET_TARGET_USER
    )

    # --------------------------------------------------
    # Validate user is not blocked before generation
    # --------------------------------------------------

    assert_no_member_authorization_block(page)
    ...
    # --------------------------------------------------
    # Validate Generated Password Policy
    # --------------------------------------------------
    ...
    page.screenshot(
        path="runtime/screenshots/tc022_safe_validation_passed.png"
    )

    log(
        "OTCT-7968_TC_022 PASSED - Authorized PasswordResetUser "
        "can access password reset for same-Member-ID user and "
        "generate a compliant password. Submit was intentionally "
        "not clicked to avoid permanently changing the target user's password."
    )
