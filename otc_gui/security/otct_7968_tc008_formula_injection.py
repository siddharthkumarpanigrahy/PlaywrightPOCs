...
FORMULA_INJECTION_FILE = (
    ...
    "portfolio transfer/Portfolio_FormulaInjection.csv"
)
...
def confirm_mtm_if_present(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc008_mtm_confirmation_popup.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc008_mtm_popup_no_button_found.png"
    )
...
def check_storage_error_after_upload(page):
    if page.get_by_text(
        ...
        page.screenshot(
            path="runtime/screenshots/tc008_file_not_found_in_storage.png"
        )

        raise Exception(
            "File upload failed before formula-injection validation: "
            "File not found in storage"
        )


def validate_formula_injection_handling(page):
    ...
    # Case 1: Application shows a validation popup/message.
    if any(keyword in body_text_lower for keyword in VALIDATION_KEYWORDS):
        if (
            ...
            or page.get_by_text("Trade Ids should not contain", exact=False).count() > 0
        ):
            page.screenshot(
                path="runtime/screenshots/tc008_validation_popup_displayed.png"
            )

            log(
                "Formula-injection file triggered validation popup/message"
            )
    ...
    # Case 2: Application creates a result grid row.
    ...
    if result_row_count > 0:
        ...
        if upload_status in ("SUCCESS", "FAILURE"):
            page.screenshot(
                path="runtime/screenshots/tc008_grid_result_row.png"
            )

            log(
                "Formula-injection file was handled safely "
        ...
        raise Exception(
            "Formula-injection file produced grid row, "
            f"but upload status was unexpected: [{upload_status}]"
        )

    page.screenshot(
        path="runtime/screenshots/tc008_no_validation_or_grid_result.png"
    )

    raise Exception(
        "No validation popup and no result grid row found "
        "for CSV formula-injection input file"
    )
...
try:
    log("Starting OTCT-7968_TC_008")

    if not os.path.exists(FORMULA_INJECTION_FILE):
        raise Exception(
            f"Missing test data file: {FORMULA_INJECTION_FILE}"
        )
    ...
    page.screenshot(
        path="runtime/screenshots/tc008_after_login.png"
    )
    ...
    # --------------------------------------------------
    # Upload Formula Injection CSV File
    # --------------------------------------------------

    log("Preparing to upload CSV formula-injection file")

    log("Uploading CSV formula-injection file")

    page.locator(
        PortfolioTransferLocators.FILE_UPLOAD
    ).set_input_files(FORMULA_INJECTION_FILE)
    ...
    page.screenshot(
        path="runtime/screenshots/tc008_after_formula_injection_file_upload.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc008_after_book_auto_population.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc008_after_mtm_yes_selection.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc008_after_create_click.png"
    )

    # --------------------------------------------------
    # Validate CSV Formula Injection Handling
    # --------------------------------------------------

    validation_result = validate_formula_injection_handling(page)

    log(
        "TC008 PASSED - CSV formula-injection input was handled safely. "
