...
DUPLICATE_TRADE_ID_FILE = (
    "./test_data/otc_gui/security/"
    "portfolio transfer/Portfolio_DuplicateTradeIds_WithSpaces.csv"
)
...
def confirm_mtm_if_present(page):
    ...
    page.screenshot(
        path="runtime/screenshots/tc026_mtm_confirmation_popup.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc026_mtm_popup_no_button_found.png"
    )
...
def check_storage_error_after_upload(page):
    if page.get_by_text(
        ...
        page.screenshot(
            path="runtime/screenshots/tc026_file_not_found_in_storage.png"
        )

        raise Exception(
            "File upload failed before duplicate Trade ID validation: "
            "File not found in storage"
        )


def validate_duplicate_trade_id_handling(page):
    ...
    # Case 1: Application shows validation popup/message.
    if any(keyword in body_text_lower for keyword in VALIDATION_KEYWORDS):
        if (
            ...
            or page.get_by_text("Duplicate", exact=False).count() > 0
            or page.get_by_text("Csv", exact=False).count() > 0
            or page.get_by_text("Trade", exact=False).count() > 0
        ):
            page.screenshot(
                path="runtime/screenshots/tc026_validation_popup_displayed.png"
            )

            log(
                "Duplicate Trade ID file triggered validation popup/message"
            )
    ...
    # Case 2: Application creates result grid row with FAILURE.
    ...
    if result_row_count > 0:
        ...
        if upload_status == "FAILURE":
            page.screenshot(
                path="runtime/screenshots/tc026_failure_grid_row.png"
            )

            log(
                "Duplicate Trade ID file was rejected "
                "with FAILURE grid row"
            )

            return "GRID_FAILURE"

        raise Exception(
            "Duplicate Trade ID file was processed, "
            f"but upload status was not FAILURE. Status=[{upload_status}]"
        )

    page.screenshot(
        path="runtime/screenshots/tc026_no_validation_or_grid_result.png"
    )

    raise Exception(
        "No validation popup and no FAILURE result row found "
        "for duplicate Trade ID input file"
    )
...
try:
    log("Starting OTCT-7968_TC_026")

    if not os.path.exists(DUPLICATE_TRADE_ID_FILE):
        raise Exception(
            f"Missing test data file: {DUPLICATE_TRADE_ID_FILE}"
        )
    ...
    # --------------------------------------------------
    # Upload Duplicate Trade ID CSV File
    # --------------------------------------------------

    log("Preparing to upload duplicate Trade ID file")

    log("Uploading duplicate Trade ID file")

    page.locator(
        PortfolioTransferLocators.FILE_UPLOAD
    ).set_input_files(DUPLICATE_TRADE_ID_FILE)
    ...
    page.screenshot(
        path="runtime/screenshots/tc026_after_duplicate_trade_id_file_upload.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc026_after_book_auto_population.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc026_after_mtm_yes_selection.png"
    )
    ...
    page.screenshot(
        path="runtime/screenshots/tc026_after_create_click.png"
    )

    # --------------------------------------------------
    # Validate Duplicate Trade ID Handling
    # --------------------------------------------------

    validation_result = validate_duplicate_trade_id_handling(page)

    log(
        "TC026 PASSED - Duplicate Trade IDs with spaces "
        "were handled safely. "
